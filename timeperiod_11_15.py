...
df_11_15 = pd.read_csv (r'DAPPER_11_15\persona_topic.csv')
drop_optn_11_15 = ['Persona0', 'Persona1', 'Persona2', 'Persona3']

df_auth_per_11_15 = pd.read_csv (r'DAPPER_11_15\author_persona.csv')
authors_list_11_15 = df_auth_per_11_15['author']

# df_sim = pd.DataFrame( columns=['Source_Author', 'Target_Author', 'Similarity'])
auth_id_doc_map_11_15 = pd.read_csv (r'DAPPER_11_15\auth_id_doc_map.csv')

auth_id_doc_map_11_15 = auth_id_doc_map_11_15.sort_values(by=['Size'],ascending=False)
authors_dict_11_15=dict([(i,s) for i,s in zip(auth_id_doc_map_11_15.Id, auth_id_doc_map_11_15.Name)])
#----------------------------------------------------------------------------------------------------------
default_stylesheet = [
    {
        'selector': 'node',
...
def layout():
    ...
    return html.Div([
        html.Div([
            html.H3('Dapper-Variant - II : Analysis and visualizations for 2011-2015',style={'textAlign': 'center'}),
        ...
        html.Div([
            ...
            dcc.Dropdown(
                id='demo-dropdown_11_15',
                value='Persona0',
                options=[{'label': i, 'value': i} for i in drop_optn_11_15]
            ),
        ...
        html.Div([
            ...
            dcc.Dropdown(
                id='demo-dropdown-authors-11-15',
                value=243,
                options=[{'label': authors_dict_11_15[i], 'value': i} for i in authors_dict_11_15]
            ),
        ...
        html.Div(children=[
            html.Iframe(src=app.get_asset_url('collapsible_tree_vis_11_15.html'),
...
@app.callback(
    dash.dependencies.Output('persona-topic-11-15', 'figure'),
    [dash.dependencies.Input('demo-dropdown_11_15', 'value')])
def update_persona_evol(value):
    ...
    print(value)
    if(value == 'Persona0'):
        df_0 = df_11_15[df_11_15['persona']==0]
        ...
        df_1 = df_11_15[df_11_15['persona']==1]
        for i,j in zip(topics,colors):
            traces.append(
                go.Scatter(
                    fill= 'tonexty',
        ...
        df_2 = df_11_15[df_11_15['persona']==2]
        for i,j in zip(topics,colors):
            traces.append(
                go.Scatter(
                    fill= 'tonexty',
        ...
        df_3 = df_11_15[df_11_15['persona']==3]
        for i,j in zip(topics,colors):
            traces.append(
                go.Scatter(
                    fill= 'tonexty',
        ...
        df_4 = df_11_15[df_11_15['persona']==4]
        for i,j in zip(topics,colors):
            traces.append(
                go.Scatter(
                    fill= 'tonexty',
...
@app.callback(
    dash.dependencies.Output('donut_sim-11-15', 'figure'),
    [dash.dependencies.Input('demo-dropdown-authors-11-15', 'value')])
def persona_dist(value):
    traces=[]
    auth_dist = df_auth_per_11_15.loc[df_auth_per_11_15.author == value, ['persona0','persona1','persona2','persona3']].values.flatten().tolist()
    ...
    return {
        ...
        'layout':go.Layout(
            ...
            annotations=[dict(text=authors_dict_11_15[value], x=0.18, y=0.5, font_size=13, showarrow=False)]
        )
    }

@app.callback(Output('cytoscape-dap-nodes-11-15', 'elements'),
              Input('demo-dropdown-authors-11-15', 'value'))
def update_elements(value):
    datas=[]
    s=[]
    src_vec = df_auth_per_11_15.loc[df_auth_per_11_15.author == value, ['persona0','persona1','persona2','persona3']].values.flatten().tolist()
    s=src_vec
    for index,i in df_auth_per_11_15.iterrows():
        tar_vec = [i['persona0'],i['persona1'],i['persona2'],i['persona3']]
        sim = 1 - spatial.distance.cosine(s, tar_vec)

        doc_source=auth_id_doc_map_11_15.loc[auth_id_doc_map_11_15['Id'] == value, 'Docs'].tolist()
        doc_target=auth_id_doc_map_11_15.loc[auth_id_doc_map_11_15['Id'] == i['author'], 'Docs'].tolist()
    ...
    merged_tab=pd.merge(df_sim2, auth_id_doc_map_11_15, on='Id', how='inner')
    Result = pd.DataFrame(merged_tab)
    # Result.to_csv("C:/Users/Subhashree/Documents/KMD_Thesis/Complete Code Collection/DAPPER/Results of DAPPER/2011_2015/AUTH/Result.csv")
...
#               Input('cytoscape-dap-nodes-00-05', 'tapNodeData'))
