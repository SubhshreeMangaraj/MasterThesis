...
df_00_05 = pd.read_csv (r'DAPPER_00_05\persona_topic.csv')
drop_optn = ['Persona0', 'Persona1', 'Persona2', 'Persona3']
# df_auth_per_00_05 = pd.read_csv (r'C:\Users\Subhashree\Documents\KMD_Thesis\Data Final\DAPPER\common_vocab\2000_2005\author_persona.csv')
df_auth_per_00_05 = pd.read_csv (r'DAPPER_00_05\author_persona.csv')
authors_list1=[]
authors_list1 = df_auth_per_00_05['author']
df_sim1 = pd.DataFrame( columns=['Source_Author', 'Target_Author', 'Similarity'])
# auth_id_doc_map_00_05 = pd.read_csv (r'C:\Users\Subhashree\Documents\KMD_Thesis\Data Final\DAPPER\common_vocab\2000_2005\auth_id_doc_map_00_05.csv')
auth_id_doc_map_00_05 = pd.read_csv (r'DAPPER_00_05\auth_id_doc_map_00_05.csv')
# auth_id_doc_map_00_05.sort(auth_id_doc_map_00_05['Size'],ascending=False)
auth_id_doc_map_00_05 = auth_id_doc_map_00_05.sort_values(by=['Size'],ascending=False)
authors_dict=dict([(i,s) for i,s in zip(auth_id_doc_map_00_05.Id, auth_id_doc_map_00_05.Name)])
#----------------------------------------------------------------------------------------------------------
default_stylesheet = [
    {
        'selector': 'node',
...
def layout():
    ...
    return html.Div([
        html.Div([
            html.H3('Dapper-Variant - II : Analysis and visualizations for 2000-2005',style={'textAlign': 'center'}),
        ...
        html.Div([
            ...
            dcc.Dropdown(
                id='demo-dropdown-authors-00-05',
                value=243,
                options=[{'label': authors_dict[i], 'value': i} for i in authors_dict]
            ),
        ...
        html.Div(children=[
            html.Iframe(src=app.get_asset_url('collapsible_tree_vis_00_05.html'),
...
@app.callback(
    dash.dependencies.Output('persona-topic-00-05', 'figure'),
    [dash.dependencies.Input('demo-dropdown_00_05', 'value')])
def update_persona_evol_scatter(value):
    ...
    if(value == 'Persona0'):
        df_0 = df_00_05[df_00_05['persona']==0]
        ...
        df_1 = df_00_05[df_00_05['persona']==1]
        for i,j in zip(topics,colors):
            traces.append(
                go.Scatter(
                    fill= 'tonexty',
        ...
        df_2 = df_00_05[df_00_05['persona']==2]
        for i,j in zip(topics,colors):
            traces.append(
                go.Scatter(
                    fill= 'tonexty',
        ...
        df_3 = df_00_05[df_00_05['persona']==3]
        for i,j in zip(topics,colors):
            traces.append(
                go.Scatter(
                    fill= 'tonexty',
        ...
        df_4 = df_00_05[df_00_05['persona']==4]
        for i,j in zip(topics,colors):
            traces.append(
                go.Scatter(
                    fill= 'tonexty',
...
@app.callback(
    dash.dependencies.Output('donut_sim-00-05', 'figure'),
    [dash.dependencies.Input('demo-dropdown-authors-00-05', 'value')])
def persona_dist(value):
    traces=[]
    auth_dist = df_auth_per_00_05.loc[df_auth_per_00_05.author == value, ['persona0','persona1','persona2','persona3']].values.flatten().tolist()
    ...
    return {
        ...
        'layout':go.Layout(
            ...
            annotations=[dict(text=authors_dict[value], x=0.18, y=0.5, font_size=13, showarrow=False)]
        )
    }

@app.callback(Output('cytoscape-dap-nodes-00-05', 'elements'),
              Input('demo-dropdown-authors-00-05', 'value'))
def update_elements(value):
    datas=[]
    s=[]
    src_vec = df_auth_per_00_05.loc[df_auth_per_00_05.author == value, ['persona0','persona1','persona2','persona3']].values.flatten().tolist()
    s=src_vec
    for index,i in df_auth_per_00_05.iterrows():
        tar_vec = [i['persona0'],i['persona1'],i['persona2'],i['persona3']]
        sim = 1 - spatial.distance.cosine(s, tar_vec)

        doc_source=auth_id_doc_map_00_05.loc[auth_id_doc_map_00_05['Id'] == value, 'Docs'].tolist()
        doc_target=auth_id_doc_map_00_05.loc[auth_id_doc_map_00_05['Id'] == i['author'], 'Docs'].tolist()
    ...
    merged_tab=pd.merge(df_sim2, auth_id_doc_map_00_05, on='Id', how='inner')
    Result = pd.DataFrame(merged_tab)
    # Result.to_csv("C:/Users/Subhashree/Documents/KMD_Thesis/Complete Code Collection/DAPPER/Results of DAPPER/2000_2005/AUTH/Result.csv")
